    ...
    useEffect(() => {
        ...
        if(userId){
            const getExpenses = async() => {
                try {
                    const axiosRequest = await axios.get("http://localhost:3001/expenses");
                        ...
                        {expenses.map((expense, i) => (
                            <div key={i} className="w-60 h-[400px] rounded-md shadow-lg">
                                ...
                                <div className="w-full h-[25%] border-t border-t-gray-100 flex items-center justify-around">
                                    <p className="text-sm font-normal text-slate-700">{expense?.createStamp}</p>
                                    <p className="text-sm font-normal text-slate-700">{expense?.amount}</p>
                                </div>
                            </div>
                        ))}
                    </>) : (<>
                ...
                {menu && (<>
                    <motion.div variants={expensesVariant} initial="initial" animate="animate" transition={transitions} className="fixed right-14 bottom-32 w-[60px] h-[150px] flex flex-col justify-evenly">
                        <div className="w-full h-[60px] rounded-full">
                            <motion.div variants={svgVariants} initial="initial" animate="animate" transition={svgTransitions} className="w-[95%] h-[95%] shadow-2xl rounded-full flex items-center justify-center cursor-pointer">
